...
// Function prototypes
//int getValues();	
void getValues(int &, int &, int &, int &, int &);	 // pass by reference
float calcAverage(int, int, int, int, int);			
void  findLowest(int, int, int, int, int, int &);			

int main()
{
	int test1, test2, test3, test4, test5, low;    // 5 user input test scores
	float avg;

	// Call getValues function to read values into the 5 test variables.
	//test1 = getValues();
	//test2 = getValues();
	//test3 = getValues();
	//test4 = getValues();
	//test5 = getValues();
	getValues(test1,test2,test3,test4,test5);  // pass by reference

	// Call calcAverage function to calculate and display the average
	avg = calcAverage(test1, test2, test3, test4, test5);
	findLowest(test1, test2,test3,test4,test5, low);
...
float calcAverage(int s1, int s2, int s3, int s4, int s5)
{
	...
	findLowest(s1, s2, s3, s4, s5, lowScore);
...
void findLowest(int s1, int s2, int s3, int s4, int s5, int & lowest)
{
	lowest = s1;
