...
int main()
{
	...
	res = searchList2(tests, SIZE, num);
	if (!res)
		cout << "You did not earn " << num << "  points on any test.\n";
	else
		cout << "You earned " << num << " points on a test.\n ";
...
// This function performs a linear search on an integer array.   *
// The list array, which has numElems elements, is searched for  *
// the number stored in value. If the number is found, ita value  *
// of true is returned. Otherwise, false is returned.             *
//****************************************************************

bool searchList2(int list[], int numElems, int value)
{
	int index = 0;               // Used as a subscript to search array 
	bool found = false;          // Flag to indicate if the value was found 
