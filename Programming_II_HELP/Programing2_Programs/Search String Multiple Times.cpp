//  Chapter 12 -- Assignment 13:  Phone Number List
...
int main()
{
   ...
   for (int k = 0; k < SIZE; k++)
     {
		 //found = phoneDirectory[k].find(name.data(), 0);
	    found = phoneDirectory[k].find(name, 0);
		while (found < phoneDirectory[k].npos)
        {
           numberEntriesFound ++;
           cout << phoneDirectory[k] << " at " << found << endl;
			found = phoneDirectory[k].find(name, found+1);
        }
     }
   if (numberEntriesFound == 0)
      cout << "\nNo entries were found for " << name << endl;
