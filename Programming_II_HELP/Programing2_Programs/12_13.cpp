...
int main()
{
   ...
   for (int k = 0; k < SIZE; k++)
     {  //cout << phoneDirectory[k] << "  " << phoneDirectory[k].npos << endl;
//       if (phoneDirectory[k].find(name.data(), 0) < phoneDirectory[k].npos)
 //      if (phoneDirectory[k].find(name.data(), 0) < phoneDirectory[k].length())
       if (phoneDirectory[k].find(name, 0) < phoneDirectory[k].npos)
        {
           numberEntriesFound ++;
