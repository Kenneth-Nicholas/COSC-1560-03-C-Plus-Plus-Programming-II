...
void backupLines(fstream &in, int lines);  // Prototype
...
int main()
{
    ...
    // Open the file
	fstream inputFile;
	inputFile.open(fileName.data(), ios::in);
    if (!inputFile)
    {
...
void backupLines(fstream &in, int lines)
{
   char ch;
   ...
   while (true)
   {
     ...
     long pos = in.tellg();
     if (pos <= 1) return;  /// tellg never actually gets to 0;
