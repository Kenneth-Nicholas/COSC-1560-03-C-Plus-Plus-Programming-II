//  Written by C++ Programmer
//  Chapter 10
//  Assignment 2:  Drop lowest score Test Averaging

#include <iostream>
#include <iomanip>
using namespace std;

void sort(float*, int);  
float dropLowestsAverage(float *, int);
void getTestData( float *, int);

int main()
{
	int numElements;
//	const int numElements = 4;
//	float grades[numElements]; // static array
	float avg = 0.0;
	
	cout << "Please enter the number of test scores: ";
	cin >> numElements;
	while(numElements <=0)
	{
		cout << "Please enter the number of test scores: ";
		cin >> numElements;
	}

	float * grades = new float[numElements]; // dynamic array

	if(grades == NULL)
	{
		cout << "Could not create an array of size " << numElements << endl;
		exit (-1);
	}
	getTestData(grades, numElements);

	sort(grades, numElements);
	avg = dropLowestsAverage(grades, numElements);
	cout << "\nThe test scores, and their average, are: " << endl << endl;
	cout << "\n score" << endl;
	cout << " -----" << endl;
	for (int j = 1; j < numElements; j++)
	{
	  cout << "\n" << fixed << setprecision(2) << setw(6) << *(grades+j);
	}
	cout << "\n\nAverage score:  " << setprecision(2)<< fixed << avg << endl << endl;

	delete [] grades;
	grades = 0;
	return 0;
}

void getTestData( float * score, int elems)
{
	for (int i = 0; i < elems; i++)  // get scores
    {
		cout << "Enter a test score:  ";
//        cin >> score[i];
        cin >> *(score+i);
        while (*(score+i) < 0)  // no negative scores
//        while (score[i] < 0)  // no negative scores
        {
			cout << "score cannot be negative" << endl;
            cout << "Enter another number:  ";
           cin >> *(score+i);
//          cin >> score[i];
		}
	}
}




/***************************************************
* sorts an array score of size elems               *
****************************************************/

void sort(float* score, int elems)
{
	int startScan, minIndex; 
	float minValue;

	for (startScan = 0; startScan < (elems - 1); startScan++)
	{
		minIndex = startScan;
//		minValue = score[startScan];
		minValue = *(score+startScan);
		for(int index = startScan + 1; index < elems; index++)
		{
			if (*(score+index) < minValue)
			{
//				minValue = score[index];
				minValue = *(score+index);
				minIndex = index;
			}
		}
		*(score+minIndex) = *(score+startScan);
		*(score+startScan) = minValue;
	}
}

/*****************************************************
* computes average of entries in an array score      *
* with numScores entries                             *
******************************************************/
float dropLowestsAverage(float* score, int numScores)
{
	float total = 0;
	for (int k = 1; k < numScores; k++)
	{
//		total += score[k];
		total += *(score+k);
	}
	return (total / (numScores-1));
}